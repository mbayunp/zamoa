...
const Struktur = () => {
  return (
    <div className="min-h-screen bg-[#F9FAFB] font-sans animate-fade-in">
      
      {/* --- HERO HEADER --- */}
      <section className="relative py-20 bg-gradient-to-br from-[#0A0A44] to-[#006B3F] text-white text-center overflow-hidden">
        {/* Dekorasi Background */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full blur-3xl -translate-y-10 translate-x-10"></div>
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#F15A24]/10 rounded-full blur-3xl translate-y-10 -translate-x-10"></div>
        
        <div className="relative z-10 container mx-auto px-4">
          <h1 className="text-4xl md:text-5xl font-black mb-2 tracking-tight uppercase">STRUKTUR ORGANISASI</h1>
          <h2 className="text-lg md:text-xl font-bold text-[#FBB03B] uppercase tracking-wider">Yayasan Zamoa Peduli Bangsa</h2>
          <div className="inline-block mt-4 px-4 py-1.5 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-xs font-bold uppercase tracking-wider">
      ...
      <div className="container mx-auto px-4 py-16 space-y-20 relative">
        ...
        <div className="absolute left-1/2 top-40 bottom-20 w-0.5 bg-slate-200 -translate-x-1/2 hidden lg:block -z-10"></div>

        {/* --- 1. ORGAN UTAMA (Petinggi) --- */}
        <div className="relative">
          <SectionTitle icon={<ShieldCheck size={18} />} title="Organ Utama Yayasan" color="bg-[#0A0A44]" />
          ...
          <div className="flex justify-center mb-12 relative z-10">
             ...
             <div className="absolute -top-12 w-0.5 h-12 bg-slate-200 hidden md:block"></div>
             
             <div className="w-full max-w-sm transform hover:scale-105 transition-transform duration-300">
                <div className="absolute -inset-1 bg-gradient-to-r from-[#0A0A44] to-[#F15A24] rounded-[1.8rem] blur opacity-25"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto relative z-10">
            ...
            <div className="absolute -top-8 left-1/4 right-1/4 h-0.5 bg-slate-200 hidden md:block border-t border-dashed border-slate-300"></div>
            <div className="absolute -top-8 left-1/2 h-8 w-0.5 bg-slate-200 hidden md:block -translate-x-1/2"></div>
        ...
        <div className="relative">
          <SectionTitle icon={<Zap size={18} />} title="Unit Pendukung Operasional" color="bg-[#006B3F]" />
        ...
        <div className="relative">
          <SectionTitle icon={<Briefcase size={18} />} title="Bidang Program Utama" color="bg-[#F15A24]" />
...
const SectionTitle = ({ icon, title, color }) => (
  <div className="flex items-center justify-center mb-10 relative z-10">
    <div className={`flex items-center gap-3 px-6 py-2.5 rounded-full text-white font-black text-xs uppercase tracking-wider shadow-md ${color}`}>
      {icon}
      <span>{title}</span>
    </div>
  </div>
